...
export function PetHealthPageClient({ petId }: { petId: string }) {
      ...
      <div className="space-y-0">
        {pet ? (
          <div className="relative -mx-2 overflow-hidden rounded-[30px]">
            ...
            <Link href={`/pets/${petId}`} className="absolute left-5 top-5 flex h-12 w-12 items-center justify-center rounded-full bg-black/35 text-white backdrop-blur">
        ...
        <section className="bg-pawbit-background px-2 pb-6 pt-5">
          <div className="rounded-pill bg-pawbit-error-bg px-1 py-1">
            <div className="grid grid-cols-3 gap-1">
              <Link href={`/pets/${petId}`} className="rounded-pill px-4 py-3 text-center text-[16px] font-medium text-pawbit-primary">
                Resumen
              </Link>
